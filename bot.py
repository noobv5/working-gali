...
async def send_all_with_timeout(username, messages):
    tasks = [asyncio.create_task(send_from_session(s, username, messages, i+1))
...
async def handle_send(client: Client, message: Message):
    ...
    if len(parts) != 2:
        await message.reply("⚠️ সঠিক ব্যবহার: /send @username")
        return

    username = parts[1].replace("@", "")
    messages = load_messages()
    await message.reply(f"🚀 @{username} কে ৩টি session দিয়ে মেসেজ পাঠানো শুরু হলো (১ মিনিট সময়সীমা)")
